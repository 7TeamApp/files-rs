...
const getRandText = jest.fn((length: number): string =>
    crypto.randomBytes(length).toString('hex').slice(0, length)
);

const getRandNum = jest.fn(
    (min: number, max: number): number =>
        Math.floor(Math.random() * (max - min + 1)) + min
);

const getRandPath = jest.fn(
    (count: number, min: number, max: number): string[] => {
...
beforeEach(() => {
    ...
    copiedDistFolder = join(distFolder, ...getRandPath(getRandNum(1,9), 2, 20));
...
const cases = new Array(rerunEach).fill(undefined);

test.each(cases)('copy one file relative', () => {
